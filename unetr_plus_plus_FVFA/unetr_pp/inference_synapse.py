...
def test(fold): 
    label_path= "/mnt/lustre/users/sntanzi/unetr_plus_plus/DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task002_Synapse/"
    infer_path = "/mnt/lustre/users/sntanzi/unetr_plus_plus_6/unetr_pp/evaluation/unetr_pp_synapse_checkpoint/unetr_pp/3d_fullres/Task002_Synapse/unetr_pp_trainer_synapse__unetr_pp_Plansv2.1/fold_0/"  
    
    label_list=sorted(glob.glob(os.path.join(label_path,'labelsTs','*nii.gz')))
    infer_list=sorted(glob.glob(os.path.join(infer_path,'validation_raw_postprocessed','*nii.gz')))

    ...
    def __init__(self, input_size=[(32, 32, 32), (16, 16, 16), (8, 8, 8), (4, 4, 4)],dims=[32, 64, 128, 256],
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                flattened_input = input_size[i][2] * input_size[i][1] * input_size[i][0]
                stage_blocks.append(TransformerBlock(input_size=flattened_input, hidden_size=dims[i], num_heads=num_heads,
                                                     dropout_rate=transformer_dropout_rate, pos_embed=True, input_resolution=input_size[i]))
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(input_size=out_size, hidden_size= out_channels, num_heads=num_heads,
                                                     dropout_rate=0.1, pos_embed=True, input_resolution=output_resolution))

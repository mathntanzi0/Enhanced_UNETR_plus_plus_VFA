...
// Forward kernel for 3D
template <typename scalar_t>
__global__ void av_bw_kernel_3d(
    ...
    if (x < (d_output.size(0) * d_output.size(1))){
        ...
        if (y < d_output.size(2)){
            ...
            if (z < kernel_size * kernel_size * kernel_size){
                ...
                const int kd = z / (kernel_size * kernel_size);
                const int rem = z - kd * (kernel_size * kernel_size);
                const int ki = rem / kernel_size;
                const int kj = rem - ki * kernel_size;
                const int i = y / (width * depth);
                const int rem_y = y - i * (width * depth);
                const int j = rem_y / depth;
                const int k = rem_y - j * depth;
                const int ni = i + ki - (kernel_size - 1) / 2;
                const int nj = j + kj - (kernel_size - 1) / 2;
                const int nk = k + kd - (kernel_size - 1) / 2;
...
// Backward kernel for 3D
template <typename scalar_t>
__global__ void av_inverse_bw_kernel_3d(
...
std::vector<torch::Tensor> av_bw_cu(
    ...
    ([&] {
        av_bw_kernel_3d<scalar_t><<<A_blocks, A_threads>>>(
        ...
        av_inverse_bw_kernel_3d<scalar_t><<<V_blocks, V_threads>>>(

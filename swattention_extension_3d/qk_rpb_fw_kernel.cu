...
__global__ void qk_rpb_fw_kernel(
    ...
    if (x < (queries.size(0) * queries.size(1))) {
        ...
        if (y < queries.size(2)) {
            ...
            if (z < kernel_size * kernel_size * kernel_size) {
                ...
                if (((ni >= 0) && (ni < height)) && ((nj >= 0) && (nj < width)) && ((nk >= 0) && (nk < depth))) {
                    const int key_y = ni * (width * depth) + nj * depth + nk;
                    #pragma unroll
...
torch::Tensor qk_rpb_fw_cu(
    ...
    TORCH_CHECK(queries.size(0) == keys.size(0), "Query and Key should have the same Batch_Size");
    TORCH_CHECK(queries.size(1) == keys.size(1), "Query and Key should have the same Head Nums");
    TORCH_CHECK(queries.size(2) == keys.size(2), "Query and Key should have the same Pixel Nums");
    TORCH_CHECK(queries.size(3) == keys.size(3), "Query and Key should have the same Head Dims");
    TORCH_CHECK(rpb.size(0) == keys.size(1), "Relative_Position_Bias should have the same Head Dims with Query and Key");

    const int B = queries.size(0), N = queries.size(1), L = queries.size(2), C = queries.size(3);

    const int attention_span = kernel_size * kernel_size * kernel_size;
    TORCH_CHECK(rpb.size(1) == attention_span, "Last dim of Relative_Position_Bias should equal Kernel_Size^3");
